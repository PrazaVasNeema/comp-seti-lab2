        ...
        public NDT.ViewData GetViewData()
        {
            ...
            foreach (var dict in researchDict)
            {
                foreach (var kvp in dict)
                {
                    ...
                    {
                        ValidateMinValue(pointData.position.x);
                        ValidateMaxValue(pointData.position.x);
                    }
                }
            ...
            foreach (var dict in researchDict)
            {
                foreach (var kvp in dict)
                {
                    foreach (var pointData in kvp.Value)
                    {
                        ...
                        float value = pointData.position.x;
            ...
            var viewData = new NDT.ViewData(NDT.TargetView.densityX);
